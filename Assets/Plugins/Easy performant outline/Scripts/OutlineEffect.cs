...
namespace EPOOutline
{
    public static class OutlineEffect
    {
        ...
        private static void Postprocess(OutlineParameters parameters, int first, int second, Material material, int iterrations, bool additionalShift, float shiftValue, ref int stencil, Rect viewport)
        {
            if (iterrations <= 0)
                return;
            ...
            for (var index = 1; index <= iterrations; index++)
            {
                ...
                parameters.Buffer.SetGlobalVector(ShiftHash, new Vector4(shift, 0));
                ...
                parameters.Buffer.SetGlobalVector(ShiftHash, new Vector4(0, shift));
        ...
        public static void SetupOutline(OutlineParameters parameters)
        {
            ...
            var scaledWidth = parameters.TargetWidth;
            var scaledHeight = parameters.TargetHeight;
            ...
            if (parameters.UseInfoBuffer)
            {
                ...
                DrawOutlineables(parameters, CompareFunction.Always, 0.0f, x => x.OutlineParameters.Enabled,   x => new Color(x.OutlineParameters.DilateShift * parameters.DilateShift, x.OutlineParameters.BlurShift * parameters.BlurShift, 0, 1),
                    x => OutlineMaterial,
                    RenderStyle.Single, 
                    OutlinableDrawingMode.Normal);

                DrawOutlineables(parameters, CompareFunction.NotEqual, 0.0f, x => x.BackParameters.Enabled,      x => new Color(x.BackParameters.DilateShift * parameters.DilateShift, x.BackParameters.BlurShift * parameters.BlurShift, 0, 1),
                    x => OutlineMaterial,
                    RenderStyle.FrontBack);

                DrawOutlineables(parameters, CompareFunction.LessEqual, 0.0f, x => x.FrontParameters.Enabled,     x => new Color(x.FrontParameters.DilateShift * parameters.DilateShift, x.FrontParameters.BlurShift * parameters.BlurShift, 0, 1),
                    x => OutlineMaterial,
                    RenderStyle.FrontBack, 
                    OutlinableDrawingMode.Normal);
                ...
                parameters.Buffer.EnableShaderKeyword(KeywordsUtility.GetSobelKeyword());
                Blit(parameters, 
                    RenderTargetUtility.ComposeTarget(parameters, InfoTargetHash), 
                    RenderTargetUtility.ComposeTarget(parameters, PrimaryInfoBufferHash),
                    RenderTargetUtility.ComposeTarget(parameters, PrimaryInfoBufferHash), 
                ...
                parameters.Buffer.DisableShaderKeyword(KeywordsUtility.GetSobelKeyword());
                parameters.Buffer.SetGlobalInt(OperationHash, (int)StencilOp.Keep);
                Blit(parameters,
                    RenderTargetUtility.ComposeTarget(parameters, InfoTargetHash),
